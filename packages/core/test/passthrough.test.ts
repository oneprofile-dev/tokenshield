...
async function mockAnthropic(handler: (chunks: Array<{ event: string; data: unknown }>) => void): Promise<{ url: string; close: () => Promise<void> }> {
  ...
  const server = createServer((req, res) => {
    ...
    req.on("end", () => {
      if (req.url === "/v1/messages/stream") {
        ...
        res.end();
        handler(events as Array<{ event: string; data: unknown }>);
      } else {
        res.statusCode = 200;
...
interface ServerLike { close(): void }
async function withTempLedger<T>(fn: (path: string) => Promise<T>): Promise<T> {
...
function doRequest(port: number, path: string, body: unknown): Promise<{ status: number; body: string }> {
  return new Promise((resolve, reject) => {
...
test("passthrough: non-streaming JSON request records usage to ledger", async () => {
  await withTempLedger(async (ledgerPath) => {
    ...
    try {
      ...
      const parsed = JSON.parse(res.body) as { usage?: { input_tokens?: number } };
